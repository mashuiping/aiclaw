...
> **Important:** `Allocated resources` on a node aggregates `nvidia.com/gpu` (vGPU **slots**) across the whole node. It does **not** prove that a **specific physical GPU UUID** still has free `nvidia.com/gpumem` / `nvidia.com/gpucores`. For per-card truth, use **D** below.
...
**D. Per-physical-GPU allocation on a node (HAMi `hami.io/vgpu-devices-allocated`):**

After scheduling, HAMi records which device(s) each Pod uses in **`hami.io/vgpu-devices-allocated`**. Use this to see **per-GPU UUID** memory/core allocation on a node—essential when debugging **`CardInsufficientMemory`**, **`CardUuidMismatch`**, or workloads that set **`nvidia.com/use-gpuuuid`**.

```bash
# Replace with your node name
NODE=gpu-3
kubectl get pods -A --field-selector spec.nodeName=$NODE \
  -o custom-columns='NS:.metadata.namespace,NAME:.metadata.name,ALLOC:.metadata.annotations.hami\.io/vgpu-devices-allocated' \
  | grep -v '<none>'
```

- **Typical annotation format:** one or more segments separated by `;`, each like `UUID,vendor,memMB,cores:` (multi-GPU Pods concatenate segments).
- **Physical GPU count on the node:** parse JSON array length in **`hami.io/node-nvidia-register`**:
  ```bash
  kubectl get node "$NODE" -o jsonpath='{.metadata.annotations.hami\.io/node-nvidia-register}' | jq 'length'
  ```
- **Cross-check pending Pods:** `kubectl describe pod` Events from **`hami-scheduler`** may report `FilteringFailed` with `CardInsufficientMemory(<node>)` when the **pinned UUID** has no remaining VRAM—even if `describe node` still shows free `nvidia.com/gpu` slots.

**Source case (human-readable walkthrough):** [2026-04-08-hami-gpu-node-aggregate-vs-per-card-pytorchjob.md](../../cases/2026-04-08-hami-gpu-node-aggregate-vs-per-card-pytorchjob.md)
